...
def get_cifar10_(root, n_labeled,
...
def get_cifar10(root, n_labeled,
    ...
    base_dataset = torchvision.datasets.CIFAR10All(root, train=True, download=download)
    train_labeled_idxs, train_unlabeled_idxs = train_split(base_dataset.targets, int(n_labeled/10))

    train_labeled_dataset = CIFAR10_labeled(root, train_labeled_idxs, train=True, transform=transform_train)
    train_unlabeled_dataset = CIFAR10_unlabeled(root, train_unlabeled_idxs, train=True, transform=TransformTwice(transform_train))
    val_dataset = CIFAR10_labeled(root, train_unlabeled_idxs, train=True, transform=transform_val, download=True)
...
# base_dataset_ = torchvision.datasets.CIFAR10(root, train=True, download=download)
# x = np.concatenate([np.asarray(base_dataset_[i][0]) for i in range(len(base_dataset_))])
# print(x.shape)
...
class CIFAR10_labeled(torchvision.datasets.CIFAR10All):
